...
class RAGSystem:
    ...
    def _init_vectorstore(self):
        ...
        if os.path.exists(self.persist_directory):
            ...
            for pdf_path in pdf_files:
                chunks = self._read_pdf_and_split(pdf_path)
                # 100개씩 나눠서 저장
    ...
    def _read_pdf_and_split(self, pdf_path, chunk_size=1000, chunk_overlap=100):
        """PDF를 읽고 텍스트 분할"""
        print(f"  📄 PDF 처리 중: {pdf_path}")
        pdf_loader = PyPDFLoader(pdf_path)
        data_from_pdf = pdf_loader.load()

        # 텍스트 전처리: 줄바꿈 제거 및 공백 정리
        import re
        for doc in data_from_pdf:
    ...
    def add_pdf_from_bytes(self, pdf_bytes, filename: str) -> bool:
        ...
        try:
            ...
            # PDF 처리 및 분할
            chunks = self._read_pdf_and_split(tmp_path)
...
def get_rag_system(persist_directory='chroma_store', pdf_directory='datafile'):
    ...
    if _rag_instance is None:
        _rag_instance = RAGSystem(persist_directory, pdf_directory)
